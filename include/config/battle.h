...
#define B_FLAG_NO_CATCHING          FLAG_BATTLE_NO_RUNNING_CATCHING     // If this flag is set, the ability to catch wild Pokémon is disabled.
#define B_FLAG_NO_RUNNING           FLAG_BATTLE_NO_RUNNING_CATCHING     // If this flag is set, the ability to escape from wild battles is disabled. Also makes Roar/Whirlwind and Teleport (under Gen8) fail.
...
#define B_FLAG_TERA_ORB_CHARGED     FLAG_SYS_TERA_ORB     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
...
#define B_VAR_DIFFICULTY            VAR_SYS_DIFFICULTY     // If not 0, you can use this var to control which difficulty version of a Trainer is loaded. This should be manually set by the developer using Script_SetDifficulty AFTER NewGameInitData has run.
...
#define B_BATTLE_SPEED VAR_SYS_BATTLE_SPEED // Usage with a variable
